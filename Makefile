...
#  protoc \
#    --go_out=. \
#    --go_opt=module=github.com/cloudfresco/sc-gs1 \
#    $(SRCPROTO)


build: 
	@echo "Building sc-gs1"	
...
test:
	@mysql -uroot -p$(SC_GS1_DBPASSROOT) -e 'DROP DATABASE IF EXISTS  $(SC_GS1_DBNAME_TEST);'
	@mysql -uroot -p$(SC_GS1_DBPASSROOT) -e 'CREATE DATABASE $(SC_GS1_DBNAME_TEST);'
	@mysql -uroot -p$(SC_GS1_DBPASSROOT) -e "GRANT ALL ON *.* TO '$(SC_GS1_DBUSER_TEST)'@'$(SC_GS1_DBHOST)';"
	@mysql -uroot -p$(SC_GS1_DBPASSROOT) -e 'FLUSH PRIVILEGES;'
	@mysql -u$(SC_GS1_DBUSER_TEST) -p$(SC_GS1_DBPASS_TEST)  $(SC_GS1_DBNAME_TEST) < sql/mysql/sc_gs1_mysql_schema.sql

	@echo "Starting tests"
	go test -v github.com/cloudfresco/sc-gs1/internal/controllers/invoicecontrollers
